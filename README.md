...
| **DeepSearch** | Narrow the question before searching (period, jurisdiction, source), then plan against the full catalogue with a more capable model | "DeepSearch" toggle |
| **Policy Analyst** | Evaluate public policy using DNFCG criteria (pertinence, efficacy, efficiency, impact, sustainability) | A step inside DeepSearch, when the question is about a specific public policy |
