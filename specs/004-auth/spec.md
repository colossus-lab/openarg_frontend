...
- **[NEEDS CLARIFICATION CL-004]** — Deploy environment split: `NEXTAUTH_URL` differs between staging and production. Exact URLs are operational detail not tracked in the code.
- **[NEEDS CLARIFICATION CL-005]** — Allowlist update process: emails are edited in the deploy env and the service restarted. Is there a documented runbook?
...
- **[DEBT-007]** — **Allowlist environment-specific mismatch**: the staging/prod split of `ALLOWED_EMAILS` + `OPEN_BETA` is not documented anywhere in the tracked code. Add a short note to each environment's deployment docs describing which toggle applies.
